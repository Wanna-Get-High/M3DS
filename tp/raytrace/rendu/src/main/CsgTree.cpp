...
CU : ray doit être une droite dans le repère de la scène (i.e. dans le repère du noeud root du CSG)
*/
void CsgTree::intersection(const Ray &ray,IntersectionArray *res) {
    ...
    if (isLeaf()) { // intersection du rayon avec une primitive (= une feuille de l'arbre csg)
        ...
        // appel résursifs pour obtenir la liste des intersections à gauche et à droite
        _left->intersection(ray,&inter_left);
        _right->intersection(ray,&inter_right);
        // faire la fusion des listes d'intersections gauche et droite (la fusion dépendra de l'opération INTERSECTION, DIFFERENCE, UNION faite au noeud).
        res->fusion(inter_left,inter_right,operation());
    }
}
...
CsgTree::CsgTree(const Matrix4 &m,CsgTree *left,CsgTree *right,EOperation op) {
...
Vector3 CsgTree::computeNormalLeaf(const Vector3 &p) {
    if (!isLeaf()) error("only compute normal on a leaf",__LINE__,__FILE__);
...
Primitive *CsgTree::primitive() const {
    if (!isLeaf()) error("get primitive on a non-primitive",__LINE__,__FILE__);
    return _primitive;
}
...
bool CsgTree::isLeaf() const {
    return _operation==LEAF;
}


void CsgTree::setNode(const Matrix4 &m,CsgTree *left,CsgTree *right,EOperation op) {
    _parent2Node.set(m);
