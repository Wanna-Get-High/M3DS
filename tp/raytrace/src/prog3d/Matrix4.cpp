...
void Matrix4::projectionDirection(const Plane &p,const Vector3 &u) {
    Vector3 n=p.normal();
    double un=u.dot(n);
    double an=p.point().dot(n);
    setRow(0,un-n.x()*u.x(),-n.y()*u.x(),-n.z()*u.x(),an*u.x());
    setRow(1,-n.x()*u.y(),un-n.y()*u.y(),-n.z()*u.y(),an*u.y());
    setRow(2,-n.x()*u.z(),-n.y()*u.z(),un-n.z()*u.z(),an*u.z());
    setRow(3,0,0,0,un);
}

void Matrix4::projectionPoint(const Plane &plane,const Vector3 &point) {
    Vector3 n=plane.normal();
    double lan=(plane.point()-point).dot(n);
    double an=plane.point().dot(n);
    double un=point.dot(n);
    setRow(0,lan+n.x()*point.x(),n.y()*point.x(),n.z()*point.x(),-an*point.x());
    setRow(1,n.x()*point.y(),lan+n.y()*point.y(),n.z()*point.y(),-an*point.y());
    setRow(2,n.x()*point.z(),n.y()*point.z(),lan+n.z()*point.z(),-an*point.z());
    setRow(3,n.x(),n.y(),n.z(),-un);
}
