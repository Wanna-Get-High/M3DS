...
void Matrix4::projectionPoint(const Plane &plane,const Vector3 &L) {
    Vector3 n=plane.normal();
    Vector3 A=plane.point();
    double LAn=(A-L).dot(n);
    double An=A.dot(n);
    double Ln=L.dot(n);
    setRow(0,LAn+n.x()*L.x(),n.y()*L.x(),n.z()*L.x(),-An*L.x());
    setRow(1,n.x()*L.y(),LAn+n.y()*L.y(),n.z()*L.y(),-An*L.y());
    setRow(2,n.x()*L.z(),n.y()*L.z(),LAn+n.z()*L.z(),-An*L.z());
    setRow(3,n.x(),n.y(),n.z(),-Ln);
}
