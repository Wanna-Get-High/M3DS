...
void Winged::catmullClark() {
  ...
  for(unsigned int i=0;i<nb_vertex;i++) {
    vertex(i)->point(vertex(i)->pointVertex());
  }
...
void Winged::deleteFace(unsigned int i) {
  error("well... not done yet...",__LINE__,__FILE__);
}
...
void Winged::drawPointFace() {
  for(unsigned int i=0;i<nbFace();i++) {
    UtilGL::draw(face(i)->pointFace());
  }
}

void Winged::drawPointVertex() {
  for(unsigned int i=0;i<nbVertex();i++) {
    UtilGL::draw(vertex(i)->pointVertex());
  }
}
...
void Winged::drawWireHide(const Color &color) {
...
void Winged::drawWire(const Color &color) {
  glPushAttrib(GL_LIGHTING_BIT | GL_LINE_BIT | GL_POLYGON_BIT);
  glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
...
Winged *Winged::clone() {
  ...
  res=new Winged();
  for(unsigned int i=0;i<nbVertex();i++) {
    res->createVertex(vertex(i)->point());
  }
  for(unsigned int i=0;i<nbFace();i++) {
    res->createFace();
  }
...
void Winged::check() {
  ...
  for(unsigned int i=0;i<nbFace();i++) {
    cout << "Face =" << i << endl;
    if (face(i)->edge()==NULL) error("edge null dans face",__LINE__,__FILE__);
    WEdge *start=face(i)->edge();
    WEdge *e=start;
    do {
      if (e==NULL) error("Succession NULL",__LINE__,__FILE__);
      cout << "Edge = " << e << endl;
      if (e->left()==face(i)) {
        e=e->succLeft();
      }
...
/*
  for(unsigned int i=0;i<nbFace();i++) {
    if (face(i)->edge()==NULL) error("face sans arête",__LINE__,__FILE__);
    ...
    e=start;
    do {
      if (e==NULL) error("SUCCESSOR NULL",__LINE__,__FILE__);
      cout << "    Edge (face " << i << " )= " << e->begin()->point() << "," << e->end()->point() << endl;
      if (e->left()==face(i)) {
        e=e->succLeft();
      }
      else
        if (e->right()!=face(i)) error("INCOHERENT",__LINE__,__FILE__); else e=e->succRight();
    } while (e!=start);
...
void Winged::drawEdgeBound(const Color &c) {
  ...
  glPopAttrib();
  for(unsigned int i=0;i<nbEdge();i++) {
    if ((edge(i)->left()==NULL)) edge(i)->right()->draw();
    if (edge(i)->right()==NULL) edge(i)->left()->draw();
  }
