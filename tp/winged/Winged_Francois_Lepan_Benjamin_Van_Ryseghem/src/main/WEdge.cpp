...
void WEdge::computePointEdge() {
  ...
  _pointEdge+=begin()->point()+end()->point();
...
void WEdge::followBoundary() {
  ...
  do {
      ...
      else error("IMPOSSIBLE",__LINE__,__FILE__);
      e->succLine(next);
      e->mark();
      e=next;
      number++;
...
Vector3 WEdge::midPoint() {
  Vector3 res;
  res=(begin()->point()+end()->point())/2.0;
  return res;
}

void WEdge::draw() {
  glBegin(GL_LINES);
  glVertex3fv(begin()->point().fv());
  glVertex3fv(end()->point().fv());
  glEnd();
}

void WEdge::computeVector() {
  _vector.setVector(begin()->point(),end()->point());
//  _vector.normalize();
}

const Vector3 &WEdge::vector() const {
  return _vector;
}

Vector3 WEdge::direction() {
  Vector3 res;
  if (_vector.length2()>0.00000001) {
    res=_vector;
    res.normalize();
    return res;
  }
...
void WEdge::drawLine() {
  ...
  glBegin(GL_LINES);
  do {
    glVertex3fv((e->begin()->point()).fv());
    glVertex3fv((e->end()->point()).fv());
    e=e->succLine();
