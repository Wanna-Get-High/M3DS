...
void MeshObj::read(const string &resourceName) {
  ...
  fstream file;
  file.open(resource.c_str(),ios::in);
  file.width(20);
  if (!file.is_open()) {
    throw Error(string("cant load file ").append(resource),__LINE__,__FILE__);
  }
...
void MeshObj::triangulate() {
  ...
  for(unsigned int i=0;i<nb;i++) {
    if (_face[i].size()>3) {
      MeshFace add;
      for(unsigned int j=0;j<_face[i].size()-3;j++) {
        add.clear();
        add.push_back(_face[i][0]);
        add.push_back(_face[i][j+2]);
        add.push_back(_face[i][j+3]);
        _face.push_back(add);
      }

      _face[i].erase(_face[i].begin()+3,_face[i].end());
    }
  }
}
