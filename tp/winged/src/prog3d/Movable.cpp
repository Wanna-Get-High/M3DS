...
Matrix4 Movable::matrix() const {
  ...
  result.mul(Matrix4::rotation(_orientation));
  result.mul(Matrix4::scaling(_scale));
  return result;
}
...
void Movable::rotate(double angle,const Vector3 &axe,EMovable f,const Vector3 &center) {
  translate(center,f);
  rotate(angle,axe,f);
  translate(-center,f);
}
