...
void GLView::updateData() {
    ...
    if(_pressLeft)      _voiture.gauche();
    if(_pressRight)     _voiture.droite();
    if(_pressForward)   _voiture.avancer();
    if(_pressBackward)  _voiture.reculer();
...
void GLView::drawGrid() {
    ...
    pasY = pasX=demiLongueur*2.0/float(nbCase); // "longueur" en x d'une case (i.e. ici 10 cases pour aller de -3 à +3)

    posY=posX=-demiLongueur;
...
void GLView::drawSquare() {
    ...
    glColor3f(0.89,0,0.12);
    glBegin(GL_POLYGON);
    glVertex3f(-1,-1,0);
    glColor3f(0.89,9,0.6);
    glVertex3f(1,-1,0);
    glColor3f(0.3,0.9,0.12);
    glVertex3f(1,1,0);
    glColor3f(0,0,0.92);
    glVertex3f(-1,1,0);
    glEnd();
...
//    glRotatef(_angleUD,1,0,0);
//    glRotatef(_angleLR,0,0,1);

//    glScalef(3,1,1);
//    drawSquare();
    //drawSecret();
    //drawQuadStripExample();
