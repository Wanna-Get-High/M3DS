...
protected:
  ...
  //! draw a square strip
  void drawStrip();
  //! draw a grid of square
  void drawGrid();
  //! draw a grid of square (call drawGrid preceded by a transform).
  void drawGround();
  //! draw the car
  void drawCar();
  ...
  //! draw something
  void drawSecret();
  //! draw two squares linked with
  void drawQuadStripExample();
...
private:
  ...
  double _angleLR;  // est modifié si appui sur left ou right
  double _angleUD;  // est modifié si appui sur up ou downs

  prog3d::Trackball _trackball; // contrôle du placement de la scène à la souris

  Voiture _voiture; // la voiture
