...
public:
  FaceBSP(ObjetBSP *owner);

  /// donne les coordonnées du sommet i directement
  inline const prog3d::Vector3 &point(unsigned int i) const {return _tabVertex[i]->point();}
  ...
  inline void normal(const prog3d::Vector3 &n) {_normal.set(n);}
	...
	ESign sign(const prog3d::Vector3 &p) const;
	...
	prog3d::Vector3 intersection(const prog3d::Vector3 &p1,const prog3d::Vector3 &p2) const;
	/// separe this selon f
	void separate(const FaceBSP &f);
