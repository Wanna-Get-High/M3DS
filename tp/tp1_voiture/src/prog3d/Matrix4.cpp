...
void Matrix4::frustum(double left,double right,double bottom,double top,double znear,double zfar) {
  Matrix4 mat;
  mat.setRow(0,2.0*znear/(right-left)    ,0                      ,(right+left)/(right-left)  ,0);
  mat.setRow(1,0                        ,2.0*znear/(top-bottom)  ,(top+bottom)/(top-bottom)  ,0);
  mat.setRow(2,0                        ,0                      ,-(zfar+znear)/(zfar-znear)     ,-2.0*zfar*znear/(zfar-znear));
  mat.setRow(3,0                        ,0                      ,-1                         ,0);

  this->mul(mat);
}

void Matrix4::ortho(double left,double right,double bottom,double top,double znear,double zfar) {
  Matrix4 mat;
  mat.setRow(0,2.0/(right-left)  ,0                 ,0                  ,-(right+left)/(right-left));
  mat.setRow(1,0                 ,2.0/(top-bottom)  ,0                  ,-(top+bottom)/(top-bottom));
  mat.setRow(2,0                 ,0                 ,-2.0/(zfar-znear)    ,-(zfar+znear)/(zfar-znear));
