...
void Quaternion::copyToAngleAxis(double *angle,Vector3 *u) {
  *angle=2.0*acos(_w);
...
Quaternion prog3d::quaternion(const Matrix4 &m) {
    Quaternion res;
    res.set(m);
    return res;
}
...
//  cout << "Quat " << *this << endl;
//  cout << "Comparison with matrix : " << endl;
  Matrix4 m;
  m.set(*this);
//  m.print("with quat");
  Vector3 o(0,0,0);
  m.set(o,i,j,k);
//  m.print("with vectors");
*/
}
...
Quaternion Quaternion::rotation(double angle,const Vector3 &u) {
	Quaternion q;
	q.setFromAngleAxis(angle,u);
	return q;
}
