...
class Matrix4 {
    ...
    /// set this matrix from the quaternion q
    ...
    /// cumulate with rotation (axis/angle in degree)
    void rotate(double angle,const Vector3 &u);

    /// composition with rotation (quaternion)
...
inline Vector4 operator*(const Matrix4 &m,const Vector4 &p) {return m.transform(p);}
