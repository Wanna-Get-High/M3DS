...
void Texture::copyFrom(GLubyte *img,unsigned int w,unsigned int h) {
  // TODO : à changer depuis passage avec QImage !!!
  throw ErrorD("dont use Texture::copyFrom");
  if (w==0) w=_width;
  if (h==0) h=_height;
...
void Texture::read(const string &resourceName) {
  string resource=resourceFile(resourceName);
  if (_img==0) {
    _img=new QImage();
  }

  QImage load;
  load.load(QString::fromStdString(resource));
  (*_img)=load.convertToFormat(QImage::Format_ARGB32);

  if (_img->isNull()) {
    throw ErrorD("file not found : read(\""+resource+"\")");
  }
...
void Texture::toGL() {
  ...
  if (_generateMipMap) {
    gluBuild2DMipmaps(GL_TEXTURE_2D,4,_width,_height,GL_BGRA,GL_UNSIGNED_BYTE,_img->bits());
  } else
    glTexImage2D(GL_TEXTURE_2D,0,4,_width,_height,0,GL_BGRA,GL_UNSIGNED_BYTE,_img->bits());
...
void Texture::read(const string &resourceName,int level) {
  ...
  QImage img;
  img.load(QString::fromStdString(resource));
...
void Texture::setAlpha(const prog3d::Color &c,double v,double radius) {
  if (_img!=NULL) {
...
void Texture::scale(float k) {
  glPushAttrib(GL_TRANSFORM_BIT);
  glMatrixMode(GL_TEXTURE);
  glScalef(1.0/30,1.0/30,1);
  glPopAttrib();
