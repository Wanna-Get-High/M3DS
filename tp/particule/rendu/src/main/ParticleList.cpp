...
void ParticleList::draw() {
  ...
  for(unsigned int i=0;i<size();i++) {
    if ((*this)[i]->alive()) {
      if (_modeParticle) (*this)[i]->drawParticle();
...
void ParticleList::drawData() {
  for(unsigned int i=0;i<size();i++) {
    Particle *p=(*this)[i];
    glColor3f(0,0,1);
    UtilGL::drawArrow(p->position(),Vector3(2.0*p->velocity()),0.2,"","Vitesse");
    glColor3f(0,1,0);
    if (p->force().length()>0.1)
    UtilGL::drawArrow(p->position(),Vector3(10.0*p->force()),0.2,"","Force");
  }
}
...
void ParticleList::maxi(unsigned int nb) {
  _nbMaxi=nb;
  _freeList.clear();
  if (_tabParticle) delete[] _tabParticle;
  _tabParticle=new Particle[nb];
  for(unsigned int i=0;i<nb;i++) {
    _tabParticle[i].texture(_texture);
