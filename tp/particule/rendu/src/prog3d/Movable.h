...
namespace prog3d {
  ...
  class Movable {
    ...
    void translate(const Vector3 &tr,EMovable f=PARENT_COORDINATE);
    void rotate(double angle,const Vector3 &axe,EMovable f=LOCAL_COORDINATE);
    void rotate(double angle, const Vector3 &axe, const Vector3 &center,EMovable f=LOCAL_COORDINATE);
    void rotate(const Quaternion &q,EMovable f=LOCAL_COORDINATE);
    void scale(const Vector3 &s,EMovable f=LOCAL_COORDINATE);
    ...
    // returns the point/direction in LOCAL (f=LOCAL_COORDINATE and u supposed to be given in PARENT) or in PARENT (f=PARENT_COORDINATE and u supposed to be given in LOCAL)
